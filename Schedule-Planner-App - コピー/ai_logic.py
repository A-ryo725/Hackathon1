def get_scheduler_system_prompt():
    return ( #AIシステムプロンプト
        ...
        "3. ルーティンの遵守: ユーザー指定のルーティン時間は絶対に変更しない。\n"
        "4. 休憩の挿入: タスクの合間には適度な『休憩』を自動的に挟む（typeは break とする）。\n"
        "5. 健康配慮: 運動系のタスクは食後すぐを避ける。\n\n"
        "【出力形式】\n"
        "出力は必ず以下のJSON形式のみで返してください（Markdownのコードブロック ```json 等は不要）。\n"
        "時間は 00:00 から 23:59 までの24時間表記で、時系列順に並べてください。\n\n"
        "{\n"
        "  \"schedule\": [\n"
        "    {\"time\": \"7:00\", \"task\": \"起床・朝食\", \"type\": \"fixed\"},\n"
        "    {\"time\": \"8:00\", \"task\": \"読書\", \"type\": \"todo\"},\n"
        "    {\"time\": \"9:00\", \"task\": \"休憩\", \"type\": \"break\"},\n"
